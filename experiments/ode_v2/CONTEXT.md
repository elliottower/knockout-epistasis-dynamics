...
Modal, `scripts/modal_ode_arm.py`.
...
1e-12 + 1e-9·max(|x|, |y|). A network that cannot be scored is not replicated. A network with
any replicate mismatch is withheld from every registered test until its cause is found and logged,
and counts toward the void rules as a network that cannot be scored. So does a network whose
spectrum is inadmissible.
...
3. With `git status` clean, run `prereg freeze` in each registration's directory, or, for a
   registration frozen before, `prereg freeze --force --access "nothing run"`. Each header names
   the plan commit, and each freeze adds a line to the registration's log.
4. Commit the three freeze headers alone.
5. Tag that commit `ode-rerun-freeze-2` with a signed annotated tag, and push the branch and the
   tag.

A first freeze, tagged `ode-rerun-freeze`, was replaced by this one before any registered run.
No solver, classifier or dynamics setting changed. The replacement removed draft and review
wording from the two configuration files' comments; launches the sensitivity runs with stored
states; keeps two legacy-audit rows out of its counts; makes the launcher and the analysis check
each run's settings and name (`scripts/registered_runs.py`); specifies how records are copied;
and pins the published ODE files and two validation reports. The diff between the two tags
shows every change, and the registrations' logs record both freezes.
...
- the three registrations frozen at one plan commit, each passing `prereg check`, with its text
  above the log equal to the plan commit's apart from the freeze header;
...
The published ODE files the legacy audit compares against (`results/ode_full/`,
`results/grn_v2/ode_full/`) are pinned the same way.

The launcher refuses a run name not listed in `scripts/registered_runs.py`, or a construction,
n_H, K, clamp value, initial states, seed, stored states, solver or classifier that differ from
its entry there. It records the tag, both commits and the run name in every record's launch
manifest. The analysis refuses a record whose manifest names another commit or run, or whose
settings differ from its run's entry, and a run directory that is missing or empty. It requires
every frozen file to equal the tagged commit, so it can be rerun from any clone.

Launches run from a worktree checked out at the tag. Nothing is committed or logged in that
worktree until every run has finished, since either would move its HEAD or change a checked path,
and a relaunch after a timeout needs the check to pass again. The primary run is launched first.
When its first record has finished,

```
uv run --with modal==1.4.3 modal run -m scripts.modal_ode_arm::launch_check \
    --run-name primary-hillcube-n10
```

prints each finished record's freeze block, run name, and whether it holds basin statistics, and
nothing from its scores. The other ten are launched once the block names the tagged commit. When
all have finished:

1. `scripts.modal_ode_arm::fetch` copies each run's records and container records, and nothing
   else, into its registration's `results/<run name>/` (commands in each registration);
2. `scripts/boolean_estimators.py`, with `--out experiments/2026-09-21_ode-primary-rerun/results`,
   writes the Boolean estimates;
3. `uv run python -m scripts.analyze_ode_rerun <primary|graded|sensitivity>` evaluates each arm.
