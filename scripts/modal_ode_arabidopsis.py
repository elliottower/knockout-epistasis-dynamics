"""Direct ODE sweep for arabidopsis_cellcycle (n=14, 16384 coalitions).

Single-function, no orchestrator pattern — safe for --detach.

Usage:
    modal run --detach scripts/modal_ode_arabidopsis.py
"""
...
app = modal.App("epistasis-bench-ode-arabidopsis", image=image)
...
def main():
    print(f"Launching ODE sweep for {MODEL_NAME} (blocks until done)")
    result = run_arabidopsis.remote()
