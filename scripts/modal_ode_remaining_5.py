"""Re-run ODE coalition sweep for 5 remaining networks, saving raw v_mean as NPZ.

Networks: arabidopsis (n=14), drosophila (n=14), fanconi/hematopoiesis/irons (n=15).
Each runs in its own container via .map().

Usage:
    modal run -d scripts/modal_ode_remaining_5.py
"""
...
app = modal.App("ode-remaining-5", image=image)
...
def main():
    ...
    print(f"Running ODE sweep for {len(REMAINING)} remaining networks")
    for m in REMAINING:
        print(f"  {m}")
    print(f"ODE params: {ODE_PARAMS}")

    fc = run_single_model.spawn(m)
    print(f"Spawned {m}, waiting for result...")
    result = fc.get(timeout=86400)
