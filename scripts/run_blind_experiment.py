"""Run the blind composition experiment on all 6 surviving GRN models.

Saves coalition tables (.npz), wiring diagrams (.json), and composition
scores (.json) to results/grn_v2/ after each model completes.
"""
...
def main():
    ...
    print(f"[{timestamp()}] Blind composition experiment: {len(SURVIVING_MODELS)} models, n_init={N_INIT}")
    ...
    for model_name in SURVIVING_MODELS:
        ...
        # Save intermediate summary after each model
