# Pre-registration: GRN Coalition Sweep v3

**Changes from v2:** G3 (saadatpour_guardcell) dropped (degenerate
copy-chain topology). Five new models added from biodivine/PyBoolNet
survey. P3 reframed from AUROC to Spearman rho on continuous pairwise
strengths. Input node exclusion protocol added. Local Fourier
extraction via WHT replaces symbolic AND-clause analysis. Spectrum
gate uses absolute energy floor (1e-4) for order-3+ structure.
